...
class SoloLatinoProvider : MainAPI() {
            ...
            doc.select("div#seasons div.se-c").flatMap { season ->
                season.select("ul.episodios li").map {
                    ...
                    val seasonEpisodeNumber =
                        it.selectFirst("div.episodiotitle div.numerando")?.text()?.split("-")?.map { num ->
    ...
    override suspend fun loadLinks(
        ...
        val pageContent = app.get(data).document

        pageContent.select("script").amap { script ->
            regex.findAll(script.data()).toList().amap {
        ...
        pageContent.selectFirst("iframe")?.attr("src")?.let { frameUrl ->
            if (frameUrl.startsWith("http")) {
                loadExtractor(frameUrl, data, subtitleCallback, callback)
            }
        }
